...
void phong(vec3 position, vec3 fragment_normal, out vec3 diffuse, out vec3 specular)
{
	...
	if (diffuse_intensity > 0.0)
	{
		...
		float specular_intensity = max(dot(reflect_light, fragment_position_to_view), 0.0);
